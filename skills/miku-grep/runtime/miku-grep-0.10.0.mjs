...
var LIMITS = {
  regexPatternLength: 1e3,
  maxDepth: 50,
  maxFileBytes: 104857600,
  maxLineChars: 1e7,
  maxFilesVisited: 1e6,
  maxDirectoriesVisited: 1e5,
  maxMatches: 1e4,
...
// dist/validation.js
function validateAndNormalize(request) {
  ...
  for (const field of ["includeFileNamePatterns", "excludeFileNamePatterns", "excludeDirNamePatterns"]) {
    if (!isStringArray(search[field]))
      return invalid("invalid_request", `search.${field} must be an array of strings`);
  }
  const output = {
  ...
  if (encoding.onDecodeError !== "skip")
    return invalid("invalid_request", "encoding.onDecodeError must be skip");
  if (!Array.isArray(encoding.rules))
    return invalid("invalid_encoding_rule", "encoding.rules must be an array");
  for (const rule of encoding.rules) {
    if (!isPlainObject(rule) || !isSupportedEncoding(rule.encoding) || !rule.pathPattern && !rule.fileNamePattern || rule.pathPattern !== void 0 && typeof rule.pathPattern !== "string" || rule.fileNamePattern !== void 0 && typeof rule.fileNamePattern !== "string") {
  ...
  const effectiveRequest = {
    ...
    search: {
      ...
      maxLineChars: search.maxLineChars,
...
function invalid(code, message, pathValue) {
  return { ok: false, code, message, path: pathValue };
}
function validateIntegerLimit(value, fieldPath, minimum, maximum, tooLargeCode) {
...
function findUnknownField(value, shape, prefix = "") {
  ...
  for (const key of Object.keys(value)) {
    ...
    if (isPlainObject(shape[key]) && key !== "rules") {
      const nested = findUnknownField(value[key], shape[key], prefix ? `${prefix}.${key}` : key);
      if (nested)
        return nested;
    }
  }
